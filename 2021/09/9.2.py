...
print(set().union(*[basins[x[1]] for x in sorted(list([(len(basins[basin]), basin) for basin in basins]), reverse=True)[:3]]))
print(math.prod(sorted(list([len(basins[basin]) for basin in basins]), reverse=True)[:3]))
print(count)

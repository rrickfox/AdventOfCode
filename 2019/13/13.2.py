    ...
    def execute_step(self):
        ...
        if opcode == 99:
            ...
            if len(self.inp) > 0:
                ...
                self.set(self.write_parameter(param_mode // 100, 1), joystick())
            self.i += 2
        elif opcode == 4:
...
def joystick():
    global paddle, ball
    return 0 if paddle == ball else (-1 if ball < paddle else 1)

while not instance.halted:
    x = instance.run_until_output()
    y = instance.run_until_output()
    tile = instance.run_until_output()

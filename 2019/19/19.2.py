...
while True:
    lowery += 1
    instance.reset(copy(data), [lowerx, lowery])
    while instance.run_until_output() == 0:
        lowerx += 1
        instance.reset(copy(data), [lowerx, lowery])
    # print(" "*lowerx, end="")
    # print("#", end="")
    uppery += 1
    upperx += 1
    instance.reset(copy(data), [upperx, uppery])
    while instance.run_until_output() == 1:
        upperx += 1
        instance.reset(copy(data), [upperx, uppery])
    upperx -= 1
    # print(" "*(upperx - lowerx - 1), end="")
    # print("#")
    upperpoints.add((upperx, uppery))
    if (lowerx + 99, lowery - 99) in upperpoints:
        break

print(lowerx * 10000 + lowery-99)
